...
parser.add_argument('--trained_model',
                    default='weights/ssd300_SIXray_10000.pth', type=str,
...
annopath = os.path.join(args.dataset_root, 'Annotation', '%s.txt')
imgpath = os.path.join(args.dataset_root, 'Image', '%s.jpg')
imgsetpath = os.path.join(args.dataset_root, '{:s}.txt')
...
def parse_rec(annopath, imgpath):
    """ Parse a PASCAL VOC xml file """
    objects = []
    # 还需要同时打开图像，读入图像大小
    img = cv2.imread(imgpath)
    height, width, channels = img.shape
    with open(annopath, "r", encoding='utf-8') as f1:
    ...
    if not os.path.isfile(cachefile):
        ...
        for i, imagename in enumerate(imagenames):
            recs[imagename] = parse_rec(
                annopath % (imagename), imgpath % (imagename))
            if i % 100 == 0:
