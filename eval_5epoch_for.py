...
annopath = os.path.join(
    args.SIXray_root, 'Anno_core_coreless_battery_sub_2000_500', '%s.xml')
imgpath = os.path.join(
    args.SIXray_root, 'cut_Image_core_coreless_battery_sub_2000_500', '%s.jpg')
...
def parse_rec(filename, imgpath):
    ...
    imagename0 = filename.replace(
        'Anno_core_coreless_battery_sub_2000_500', 'img_fold_name')
    ...
    for i in range(num_images):
        ...
        for j in range(1, detections.size(1)):
            ...
            for item in cls_dets:
                ...
                if item[4] > thresh:
                    ...
                    cv2.rectangle(
                        im_det, (item[0], item[1]), (item[2], item[3]), (0, 0, 255), 2)
        ...
        for item in gt:
            ...
            cv2.rectangle(im_det, (int(item[0] * w), int(item[1] * h)), (int(item[2] * w), int(item[3] * h)),
                          (0, 255, 255), 2)
            cv2.putText(im_det, chinese, (int(
                item[0] * w), int(item[1] * h) - 5), 0, 0.6, (0, 255, 255), 2)
